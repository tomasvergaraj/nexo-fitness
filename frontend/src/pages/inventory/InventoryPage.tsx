...
function ProductsTab() {
  ...
  const [form, setForm] = useState({
    ...
    description: '', barcode: '', initial_stock: '', min_stock: '',
  });
  ...
  const uploadImageMutation = useMutation({
    mutationFn: ({ id, file }: { id: string; file: File }) => posApi.uploadProductImage(id, file),
    onSuccess: (res) => {
      toast.success('Imagen actualizada');
      const updated = res.data as Product;
      setEditing(prev => (prev && prev.id === updated.id ? { ...prev, image_url: updated.image_url, thumb_url: updated.thumb_url } : prev));
  ...
  const deleteImageMutation = useMutation({
    mutationFn: (id: string) => posApi.deleteProductImage(id),
    onSuccess: (res) => {
      toast.success('Imagen eliminada');
      const updated = res.data as Product;
      setEditing(prev => (prev && prev.id === updated.id ? { ...prev, image_url: null, thumb_url: null } : prev));
      queryClient.invalidateQueries({ queryKey: ['pos-products'] });
      queryClient.invalidateQueries({ queryKey: ['pos-products-all'] });
    },
    onError: (err) => toast.error(getApiError(err)),
  ...
  function handleImagePick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      toast.error('Imagen excede 5 MB');
      return;
    }
  ...
  function openCreate() {
    ...
    setForm({ name: '', sku: '', price: '', cost: '', unit: 'unit', category_id: '', description: '', barcode: '', initial_stock: '', min_stock: '' });
    setModalOpen(true);
  }
  ...
  function handleSubmit() {
    ...
    if (editing) {
      ...
      if (form.initial_stock) data.initial_stock = Number(form.initial_stock);
      if (form.min_stock) data.min_stock = Number(form.min_stock);
    ...
    <div>
                    ...
                    <div className="flex items-center gap-2">
                      {p.thumb_url || p.image_url ? (
                        <img
                          ...
                          alt={p.name}
                          className="h-9 w-9 shrink-0 rounded-lg object-cover bg-surface-100 dark:bg-surface-800"
                          loading="lazy"
                        />
                      ) : (
                        <div className="h-9 w-9 shrink-0 rounded-lg bg-surface-100 dark:bg-surface-800 flex items-center justify-center">
      ...
      <Modal open={modalOpen} title={editing ? 'Editar producto' : 'Nuevo producto'} onClose={() => setModalOpen(false)}>
        <div className="space-y-4">
          {editing && (
            <div className="flex items-center gap-3 rounded-xl border border-surface-200 dark:border-surface-700 p-3">
              <div className="h-20 w-20 shrink-0 overflow-hidden rounded-lg bg-surface-100 dark:bg-surface-800 flex items-center justify-center">
                {editing.image_url ? (
                  <img src={editing.thumb_url || editing.image_url} alt={editing.name} className="h-full w-full object-cover" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <label className="btn-secondary cursor-pointer text-xs py-1.5 px-2.5 inline-flex items-center gap-1.5">
                    ...
                    {editing.image_url ? 'Cambiar' : 'Subir'}
                    <input
                      type="file"
                  ...
                  {editing.image_url && (
                    ...
                    >
                      {deleteImageMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <ImageOff size={13} />}
                      Quitar
                    </button>
                  )}
                </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            {!editing ? (
              <>
                <div>
                  <label className="text-xs text-surface-500 block mb-1">Stock inicial</label>
